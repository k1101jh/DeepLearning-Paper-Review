...
url:
- [paper](https://openreview.net/forum?id=EanCFCwAjM) (ICLR)
...
- 이 표현이 set-level trasnformers에 적합함을 관찰
...
우리는 포즈 예측 작업을 전통적으로 사용되는 전역 카메라 매개변수를 추론하는 대신, 패치별 광선 방정식을 추론하는 것으로 재구성했습니다. • 우리는 희소하게 샘플링된 뷰를 고려하여 이 표현을 추론하기 위한 간단한 회귀 기반 접근법을 제시하며, 이 간단한 접근법조차도 최신 기술을 초월하는 것을 보여줍니다. • 우리는 광선 기반 카메라 매개변수화를 통해 덴포이징 확산 모델을 학습하여 카메라에 대한 분포를 캡처하는 이 접근법을 확장하여 성능 향상을 가져옵니다.
