...
- 두 가지 기본 원칙
    ...
    2. raw interleaved documents를 생성하여 텍스트와 이미지 내용을 비롯해 비구조적 레이아웃까지 모델링하도록 촉진
...
- 일부 연구에서는 MLLM을 이용한 조건부 이미지 생성에 성공을 거둠
    - MLLM이 사전 학습된 CLIP encoder와 명시적으로 정렬되는 이산적 또는 연속적 조건부 embedding을 생성하도록 강제(Fig 1 참조)
...
- 교차된 문서 sequence $x = \{x_t\}_{t=1}^T$는 단어 $w = \{w_i\}_{i=1}^N$과 이미지 $I = \{I_k\}_{k=1}^K$를 모두 포함
...
2. $\mathcal{I}$-GPT Training
    - 이후, LLM은 $\mathcal{I}$-GPT 사전 학습을 위해 unfrozen 과정을 거침
        ...
        - text-to-image 학습을 강화하고 일부 저품질 노이즈 이미지와 텍스트의 영향을 완화
            - LAION400M에서 2M개의 페어링된 데이터 샘플을 사용
            - BLIP 자막을 제공
...
> - (c)에서는 text prompt X만 사용한 SD baseline의 결과를 보임
...
- DreamLLM에서는 conditional embedding이 일부 학습된 dream 쿼리와 함께 MLLM으로부터 유도됨
...
1. query attention은 구조화되어 있고, 분리되어 있으며(disentangled), 의미론적으로 지향적임(semantically-oriented)
...
**DreamLLM**
- multimodal content를 이해하고, Diffusion model을 통해 생성까지 가능한 MLLM을 개발하기 위한 학습 프레임워크
- conditional-image synthesis distribution의 score distillation을 통해, 중간 표현 목표가 필요하지 않음
- interleaved document의 활용은 multimodal 분포를 더욱 풍부하게 하여 multimodal encoding과 디코딩 학습을 촉진
- 다양한 VL 벤치마크에 걸친 광범위한 실험을 통해 DreamLLM의 multimodal content 이해와 생성 간의 학습 시너지를 보임
- free-form interleaved content 생성에 대한 첫걸음을 내딛음
