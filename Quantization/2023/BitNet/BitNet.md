...
ReLU와 같은 activation의 경우 activation 입력의 모든 값이 음수가 안되도록 입력 값의 최소값을 빼서 $[0, Q_b]$ 범위로 조정
...
가중치 행렬 $W \in R^{n \times m}$의 경우, partition 차원을 따라 $G$ 그룹으로 나눈다. (각 그룹의 크기: $\frac{n}{G} \times m$)
그 다음, 각 그룹에 대한 매개변수를 독립적으로 추정한다.

> $$\displaystyle \begin{aligned}
&\alpha_g = \frac{G}{nm} \sum_{ij} W_{ij}^{(g)}, \beta_g = \frac{G}{nm} \Vert W^{(g)} \Vert_1,
&(13)
\end{aligned}$$
...
> $$\displaystyle \begin{aligned}
&\gamma_b = \Vert x^{(g)} \Vert_{\infty}, \eta_g = \min_{ij} x_{ij}^{(g)}
&(14)
\end{aligned}$$
...
> **Table 1.**
BitNet과 Transformer의 에너지 소비
결과는 입력 길이 512와 같다.

![alt text](./images/Table2.png)
> **Table 2.**
45nm 및 7nm 공정 노드에서 서로 다른 bit 표현에 대한 ADD 및 MUL 에너지 소비
...
BitNet은 absmax를 사용해서 activation 양자화
